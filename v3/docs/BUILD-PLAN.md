...
3. ~~Q3 (blocks M1 scope + M7 trial economics + M9 booking): Launch surah set — the fixed floor is 12 + 112 + 103; is the revenue-path second surah Al-Mulk (narrative-adjacent, 333 words) or a Juz Amma batch (the ICP's actual repertoire and the true stress test of degenerate cases)?~~ **ANSWERED 2026-08-11 by Firdaus: AL-MULK (67).** See DECISIONS.md v3-D59. Vendored from the Quran.com API and compiled the same day — 30 ayat, 333 words, pages 562–564, cross-confirmed against QAC's independent 30/333 count. This also matches WIREFRAME §17's own named default, ending onboarding's divergence from the spec. The launch set is therefore **12 + 67 + 103 + 112**, and `scripts/content-freeze.mjs` enumerates exactly those four.
