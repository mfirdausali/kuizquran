...
| 15 | Specs / typed overrides (B1 dead) / tiered verifications | **DONE (2026-08-12)** | Schema, controllers, any-row-matches frontier all tested. **The override→re-hash trigger is now automatic** (DECISIONS.md v3-D81) — `App\Support\CorpusHashRecomputer` recomputes and re-ingests synchronously inside `OverridesController::store()`; no human re-runs `corpus:ingest-hashes` after an override. Closes DEFECTS.md#B3's last named gap. |
...
| **B3** | No content hash on verifications | **CLOSED, fully (2026-08-12)** | `hash.ts#ayahQariHashWithOverrides`; append-only `ayah_verifications`; any-row-matches frontier. The override→re-ingest trigger is now automatic (v3-D81) — `App\Support\CorpusHashRecomputer`, closing test `OverrideHashRecomputeTest`. |
