...
// look-alike similarity. Ported verbatim from
// v1/packages/corpus-compiler/src/normalize.ts (v3/CLAUDE.md's port source) —
// no surah-parameterization needed, this operates on already-extracted
// Arabic strings from the corpus, never authors any.
...
/**
 * Fold a single trailing alef that follows a consonant — the alef of
 * accusative nunation (tanwin) that indefinite-accusative nouns carry — so
 * such a distractor matches its base form in the corpus. Operates on an
 * already-normalized string. Only strips when the result stays ≥2 chars.
 */
