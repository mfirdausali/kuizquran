...
//
// v1's version parsed morphology for surah 12 only (`yusufMorph`). The QAC
// file itself already covers the whole Quran, so the only change needed for
// surah-parameterization (build-plan step 3) is to keep every surah's
// morphology, keyed by surah, instead of hardcoding "12" — this file never
// hardcoded anything Yusuf-specific about the FORMAT, only about which rows
// it kept.
...
export interface QacData {
  /** Per-surah morphology, keyed "ayah:word" within each surah's map. */
  morphBySurah: Map<number, Map<string, WordMorph>>;
  /** Normalized skeletons of every word form in the whole Quran (for validation). */
  fullQuranWordSet: Set<string>;
}

/**
 * Parse the QAC file once. Returns per-surah per-word morphology plus the
...
export function parseQac(filePath: string): QacData {
  ...
  // and (b) per-surah morphology from the head segment.
  //
  // The distractors are hand-authored Uthmani tokens whose clitic boundaries
  // differ from QAC's segmentation (e.g. QAC stores a word WITH its attached
  // definite-article clitic while a distractor gives the bare noun without
  // the article). So the "attested in the Quran" set includes, per word: the
  // whole reconstructed form, every individual segment, and every
  // leading/trailing run of segments — plus a tanwin/stem-folded variant of
  // each so accusative-nunation forms match their base (dropping the case
  // ending's trailing alef).
  ...
  const morphBySurah = new Map<number, Map<string, WordMorph>>();
  for (const [key, segs] of groups) {
    const [surahStr, ayah, word] = key.split(":");
    ...
    let bySurah = morphBySurah.get(surah);
    if (!bySurah) {
      bySurah = new Map();
      morphBySurah.set(surah, bySurah);
    }
    const h = headOf(segs);
    bySurah.set(`${ayah}:${word}`, {
  ...
  return { morphBySurah, fullQuranWordSet };
}
