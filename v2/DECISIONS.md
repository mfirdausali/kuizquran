...
## Phase 7 build decisions (executed under v2‑D31 autonomous authorization)

### v2‑D59 — GATE‑A rolling review: the autonomous build does the data‑READINESS work only; it never self‑marks `ayah_verifications` — that stays a required human action
- **When:** 2026‑07‑16 04:45:49 UTC (13:45 JST)
- **Kind:** process · product · **Status:** accepted (assistant decision under the v2‑D31 standing autonomous‑loop authorization)
- **Context:** ROADMAP Phase 7's exit criterion is "early movements verified and shippable; the verified‑frontier‑ahead‑of‑learner metric is live... no open correctness/trust item can reach a learner ahead of review." `v1/CLAUDE.md`'s GATE‑A rule is explicit and stands above phase instructions: *"Cannot self‑certify Arabic correctness."* The Phase‑6 admin console already ships a fully working `POST /admin/verifications` action and the `verifiedThrough`/`learnerFrontier`/`bufferAyat` metric (v2‑D57) — the MECHANISM for rolling review was already done; what was still open going into Phase 7 was (a) the DATA those verifications will judge, and (b) actually tapping "verify."
- **Decision:** This build does (a) only. It resolves every objectively‑fixable, non‑scholarly data gap the ROADMAP names for the early movements (12:1–20, movements 1–2) so a human qari's review isn't blocked on placeholder content, and explicitly does **not** insert any `ayah_verifications` row — that action requires an actual qari/scholar judgment call this agent cannot make, exactly as v1's GATE‑A already established for the identical class of decision. What shipped this phase:
  1. **Scene‑beat TODO labels** (19/19) replaced with authored one‑line English narrative summaries in `corpus-compiler/src/sceneBeats.ts` (regen‑safe) and patched directly into `v1/public/corpus.json` + `v2/public/corpus/12.json` (no regen run this phase). English narrative summary, not a translation/Arabic‑correctness claim — doesn't need qari review.
  2. **DATA‑1 multi‑word gloss grouping** — resolved (not just stored, v2‑D55 → v2‑D60 below) for the 10 phrases within ayat 1–20 from `v1/docs/corpus-report.md`'s reviewed table.
  3. **Null MS glosses** — machine‑sourced for all 257 words in ayat 1–20 (v2‑D27's "machine‑sourced, then qari‑verified" pipeline), via `question_overrides` rows, `editor_id:null`, explicitly noted as unverified.
  4. **~14.8% valid‑but‑attested distractors** — deliberately NOT built into new admin UI this phase (v2‑D63 below); stays a case‑by‑case qari call through the existing distractor override mechanism.
  5. **S3 dark‑mode tile contrast** — verified ALREADY fixed (v2‑D61 below); no change needed.
- **Why:** The instruction to "pick the simplest reasonable default... and proceed" governs OPEN PRODUCT/PROCESS questions, not the standing, explicitly‑stated invariant that Arabic correctness cannot be self‑certified — that rule outranks a phase's exit‑criterion wording the same way `v1/CLAUDE.md` states invariants outrank later instructions. Reading "early movements verified and shippable" as "the agent inserts verification rows" would make GATE‑A meaningless (a rubber stamp by the same system whose output it's supposed to check); reading it as "the verification MECHANISM is live and the movements are DATA‑READY for a human to actually verify quickly" satisfies the plain English of the exit criterion without violating the standing rule. The admin Frontier tab (v2‑D57) already shows `verifiedThrough:0` honestly today — that's the correct, non‑misleading state until a real qari acts.
- **Related:** v1 GATE‑A rule (`v1/CLAUDE.md`), v2‑D30 (rolling review), v2‑D57 (the frontier metric this decision deliberately leaves untouched), ROADMAP Phase 7.

### v2‑D60 — DATA‑1 `group` overrides resolved into the corpus (not just stored): `groupPositions` on `CorpusWord`, S1 probes the group once
- **When:** 2026‑07‑16 04:45:49 UTC (13:45 JST)
- **Kind:** mechanic · stack · **Status:** accepted — supersedes v2‑D55's deferral of `group` generation‑wiring
- **Context:** v2‑D55 (Phase 6) stored `group` override rows for audit/editor display but explicitly did NOT resolve them into generation, calling that "real engine/UI work... for a later pass." ROADMAP Phase 7 explicitly names DATA‑1 as something to resolve THIS phase, and inspecting the actual corpus data confirmed the mechanism needed is small: a DATA‑1 pair (e.g. 12:4 positions 8–9, أَحَدَ+عَشَرَ) already carries the IDENTICAL correct combined gloss ("eleven") on both words in the compiled corpus — the defect isn't wrong text, it's that probing both positions independently in a full S1 pass asks a near‑duplicate question about a phrase that isn't individually meaningful token‑by‑token.
- **Decision:** `overrides.ts`'s `applyOverrides()` now resolves `group` rows (payload `{groupWith: number[]}`, keyed by the override's own `position` as the anchor — the lowest position) into a new `CorpusWord.groupPositions` field, stamped on EVERY member (anchor + `groupWith`), latest‑wins per anchor like every other override field. `ladder.ts`'s S1 pass (`initLadder`/`nextItem`/`advance`) reads it: only a word that IS its own group's anchor (or is ungrouped) is ever queued/probed; the completion check (`allKnown`) and the reported `total` are computed over probeable words only. The anchor's `DrillItem` carries `groupPositions` so a future S1 hero can light every member together. Seeded via `Phase7OverrideSeeder` for the 10 real phrases in ayat 1–20 (`v1/docs/corpus-report.md`'s reviewed table), `editor_id:null`, noted as pending qari confirmation that each is a true grouping (number compound / fixed expression) and not an incidental word pair — the grouping DECISION (which pairs are true units) still needs a human per the report's own instruction ("Mark which of these runs are true units to group... vs. incidental preposition+noun pairs"); this build only wired the MECHANISM and seeded the report's own candidate list, flagged as unconfirmed.
- **Why:** No live UI currently renders a full S1 pass (v2‑D43 already noted this — Test's `vocabItem` only ever probes an ayah's FIRST word, which is never a DATA‑1 trailing member in this list), so this is engine‑layer‑correct, tested work ahead of its UI exactly like v2‑D43's own `lang` threading — not scope creep, since `ladder.ts` is independently tested (invariant #6) regardless of live callers. Reusing the EXISTING gloss‑duplication convention (both words already share the combined gloss) meant no new gloss‑patching logic was needed — only a "which positions to probe" filter, the smallest change consistent with "resolve DATA‑1."
- **Related:** v2‑D55 (superseded — group is no longer "unresolved this phase"), v1/docs/corpus-report.md (the 52‑phrase table, source of the seeded 10), invariant #6.

### v2‑D61 — S3 dark‑mode tile contrast: confirmed already fixed, no change made
- **When:** 2026‑07‑16 04:45:49 UTC (13:45 JST)
- **Kind:** ux · **Status:** accepted (verification, not a change)
- **Context:** `v1/docs/final-report.md`'s known‑gaps list named this as an open nit: "a one‑line `color: var(--text-primary)` on `.tile` in iman-ui.css (yours to make, invariant #5)."
- **Decision:** No CSS change. Both `v1/styles/iman-ui.css` and `v2/src/styles/iman-ui.css` already carry `color: var(--text-primary)` on `.tile` (with the comment "explicit — a tile's own glyph, not inherited"), and `.tile.is-ok`/`.tile.is-err` already set explicit `ok-fg`/`warn-fg` colors — someone applied this exact fix before v2's Phase 0 CSS port, ahead of the final report being updated to say so. Verified two ways: (1) computed WCAG contrast ratios against the actual dark‑mode CSS variables — default tile 10.0:1, `.is-ok` 8.1:1, `.is-err` 5.7:1 (AA requires 4.5:1) — and (2) a live `prefers-color-scheme: dark` Playwright run at a 375px viewport against `/drill?ayah=4`, screenshotted, confirming the Arabic tile glyphs and the red `.is-err` feedback state are both clearly legible.
- **Why:** Ran the actual verification instead of assuming the final‑report gap was still live — the code already matched the fix the report asked for. Recording this as its own decision (rather than silently doing nothing) keeps the audit trail honest that Phase 7 checked this item rather than skipping it.
- **Related:** `v1/docs/final-report.md` known gap #6, invariant #5.

### v2‑D62 — Fixed a real bug found while verifying MS glosses: `Test.tsx`'s vocab item never threaded the learner's chosen gloss language
- **When:** 2026‑07‑16 04:45:49 UTC (13:45 JST)
- **Kind:** mechanic · **Status:** accepted (bug found + fixed during Phase 7's own verification pass, not a pre‑planned deliverable)
- **Context:** While live‑verifying that the newly‑seeded MS gloss overrides actually reach a learner (not just sit unused in the database, the same trap v2‑D43 called out for `lang` threading generally), a Playwright run through `/test` with `iman-gloss-lang:"ms"` set showed the vocab MCQ still rendering pure English options. `engine/src/test.ts`'s `vocabItem(corpus, surah, ayah, lang = "en")` already accepted a `lang` parameter (added in v2‑D43), but `src/pages/Test.tsx`'s `buildItems()` called it as `vocabItem(corpus, SURAH, ayah)` — no fourth argument — so every Test vocab question silently defaulted to English regardless of the learner's onboarding choice. Grepping every other live call site (`Admin.tsx`'s two `wordGloss(..., "en")` calls) confirmed those are intentionally English (admin stays raw per v2‑D58) — this was the only real gap.
- **Decision:** `buildItems()` now takes an explicit `lang: GlossLang` parameter, threaded from `session/glossLang.ts`'s `getGlossLang()` at the one call site (`startTest()`), and passes it through to `vocabItem(corpus, SURAH, ayah, lang)`. Added an engine‑level regression test (`test.test.ts`) proving `vocabItem` returns the MS‑overridden gloss when `lang:"ms"` is passed, and re‑verified live: a fresh MS‑language onboarding → `/test` over 12:1–20 now shows genuine Bahasa Melayu options (e.g. "sambil menangis" / "Dan mereka datang" / "pada waktu malam" / "kepada bapa mereka" for 12:16), confirmed by screenshot with zero English markers present.
- **Why:** This directly blocked the MS‑gloss data‑sourcing work (this same phase) from ever being observable by a real learner — finding it via live verification rather than trusting the seeded data was correct is exactly the "runtime: launch the dev server and drive the affected flow" discipline the ROADMAP's Verification section requires every phase, not just the phase that first built a surface. In scope for Phase 7 as a hardening fix (a null/unused i18n path is the same class of gap as a null gloss) rather than a new phase's work.
- **Related:** v2‑D27 (bilingual glosses), v2‑D43 (built the `lang` parameter this phase finally threads all the way through), ROADMAP Phase 7, ROADMAP's Verification section.

### v2‑D63 — The ~14.8% valid‑but‑attested distractors stay a case‑by‑case qari call; no new admin UI built for them this phase
- **When:** 2026‑07‑16 04:45:49 UTC (13:45 JST)
- **Kind:** process · **Status:** accepted (scope‑narrowing decision)
- **Context:** `v1/docs/corpus-report.md` flags 1311/8880 distractors (1101 distinct forms) as morphologically‑valid but not verbatim‑attested in the mushaf — explicitly "strong pedagogical traps, not errors... flagged here for reviewer," a SOFT, non‑blocking check even in v1. The Phase‑6 admin override editor already lets a qari fully replace any word's distractor set (`field:"distractor"`) but doesn't currently surface `prd_rank`/`src_type`/`why`/attestation status inline for review.
- **Decision:** Ship no new UI for this. The existing distractor‑override mechanism (v2‑D21/D55/D56, live since Phase 6) is already sufficient for a qari to act on any specific flagged form once they reach it during rolling review; building a dedicated "attested‑ness" surfacing UI is real, scoped design/engineering work the ROADMAP doesn't name as a Phase 7 deliverable (unlike the four items in v2‑D59), and the underlying data was already explicitly soft/non‑blocking in v1.
